...
transTR2TM rep
 ...
 -- is it a Complex type? Make it a tuple!
 | isComplex = customTR2TM nestedTup `debug` (dbgStr "C")
 ...
 where (isSignal, nestedTR) = let (tc,~[tr]) = fsdSplitTyConApp rep
       ...
       (isComplex, nestedTup) = let (tc,~[tr]) = fsdSplitTyConApp rep
                                in  (tc == cpxTyCon, fsdTyConApp (fsdTupleTyCon 2) [tr,tr])
...
primTypeTable = [-- Commented out due to representation overflow
                 ...
                 (fsdTypeOf (undefined :: Int20), int20TM)   ,
                 (fsdTypeOf (undefined :: Fixed20), int20TM)   ,
...
-- preparePatNameSpace _ pat = error $ show pat
...
transExp2VHDL e@(InfixE (Just argl) f@(ConE name) (Just argr)) =
  do -- get the symbol table (name translation table)
    ...
    case lookup name nameTable of
      -- found name
      Just (2, transF) ->
        do exps <- mapM transExp2VHDL [argl, argr]
           return $ transF exps
      -- Didn't find the name in the global table
      Nothing -> expErr e $ UnkownIdentifier name
...
--transExp2VHDL exp = error $ show exp
...
cpxTyCon :: FSDTypeCon
cpxTyCon = fsdTyConOf (undefined :: Complex ())
